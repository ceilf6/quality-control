...
4. 不使用“显然”“易得”“可知”等跳步表达代替解释。公式变形、查表方法、单位换算、Excel 字段和法语术语都要说明理由。
   所有计算讲解必须完整展示“原始数据来源 → 选择该公式的理由 → 逐项代入 → 算术或代数变形 → 单位和四舍五入 → 结论”。不得将多个中间步骤压缩成一个等式，也不得在未说明 SCE、ddl、组合平均、标准差、控制限或查表参数来源时直接给出结果。
...
5. P3：`files-from-classmate/` 中的同学资料及其 readable 转换结果。仅用于中文解释、术语对照、查漏补缺和自测；不得用于决定期末范围、公式权威、TD 步骤或最终答案。若同一 TD 已有老师官方 correction，移除并不再使用对应同学答案；若暂无老师 correction，可暂用同学完成版作 P3 参考，但仍须先以 P0 CM/TD 公式和步骤核对。若与 P0/P1/P2 冲突，以老师资料为准并说明冲突。
...
讲解 TD 或综合题时必须保留原题的题号与子问编号，不得另用“第 7 部分”等可能与原题混淆的编号。每一问在计算前，必须先逐句解释法语题干、中文含义、需要交付的结果以及它与 correction 中表格或步骤的对应关系，然后才展开公式与计算。
...
当用户进入 TD 整题练习阶段时，默认只提供可在 Codex 侧边打开的老师官方 TD 原题文件或导出页链接；不在消息中转写题干、重构问题、拆解隐含子问或预先给出解题表格。链接指向的官方原题视为已展示完整题干。只有用户主动询问题措辞、法语含义或要求提示时，才针对该点解释。

自测题必须检验用户能否从完整题面中自行识别结构、选择模型、确定公式并建立表达式；不得在题目中直接给出目标公式、模型类别、代入框架或等价的解题步骤，使题目退化为纯数值代入。只有用户明确要求训练计算器操作、算术或某个指定公式的代入时，才可以直接提供公式。
...
5. 同学资料使用 `scripts/convert_classmate_materials.py` 转换，并生成：
   ...
   - 对 `.xlsx` 同时生成 `.readable.md` 和逐 sheet 的 `.html` 表格预览。
6. 转换完成后告诉用户：
   - 转换了哪个文件
   - 输出到哪里
   - 是否存在识别不确定的地方
